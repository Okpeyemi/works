...
const favoriteLinks: LinkItem[] = [
  {
    ...
    shared: true, createdAt: "Today, 10:32",
  },
  {
...
export default function FavoritesPage() {
  ...
  return (
    <DashboardShell title="Favorites" breadcrumbs={[{ label: "Home", href: "/" }, { label: "Favorites" }]}>

      {/* ── Toolbar ── */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-lg font-semibold">Favorites</h2>
          <p className="text-xs text-muted-foreground">{favoriteLinks.length} favorite links</p>
        </div>
      ...
      {viewMode === "list" && (
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="grid grid-cols-[auto_1fr_140px_120px_80px_40px] items-center gap-3 px-4 py-2.5 bg-muted/50 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground border-b border-border">
            <Checkbox
              checked={selectedLinks.size === favoriteLinks.length && favoriteLinks.length > 0}
              onCheckedChange={(checked) => {
                if (checked) setSelectedLinks(new Set(favoriteLinks.map((l) => l.id)))
